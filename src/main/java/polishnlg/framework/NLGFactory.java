...
public class NLGFactory {
	...
	private static final List<String> FIRST_PRONOUNS = Arrays.asList("ja", "my");
	private static final List<String> SECOND_PRONOUNS = Arrays.asList("ty", "wy");
	private static final List<String> PLURAL_PRONOUNS = Arrays.asList("my", "wy", "oni", "one");
	...
	protected int stringIsBaseForm(String word, LexicalCategory category) {
		...
		if (word.length() > 2) {
			if (lexicon.hasWord(word, category))
				...
				whichIsAdjective = 2;
			else if (lexicon.hasWord((word).substring(0, (word).length() - 3), category))
				whichIsAdjective = 3;
		}
	...
	private boolean stringIsWord(String string, LexicalCategory category) {
		return lexicon != null
				&& (lexicon.hasWord(string, category)
						...
						|| (string.matches(WORD_REGEX)) || string.contains("-"));
	}
	...
	public NLGElement createWord(Object word, LexicalCategory category) {
		...
		if (word instanceof NLGElement) {
			...
			if (LexicalCategory.PRONOUN == wordElement.getCategory() || LexicalCategory.POSSESSIVE_PRONOUN == wordElement.getCategory()) {
		...
		if (countBlanks == 1) {
			...
			if(part1 != null & part2 != null) {
				...
				if (stringIsWord(part2, LexicalCategory.NOUN) && adjectiveElement != null) {
					...
					adjectiveElement = createWord(part1, LexicalCategory.PREPOSITION);
